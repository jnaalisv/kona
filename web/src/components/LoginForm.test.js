...
describe('LoginForm', () => {
    ...
    it('LoginForm calls login after click', () => {
        ...
        const loginPage = mount(<LoginForm login={loginMockFunction} />);

        const usernameInput = loginPage.find('input').at(0);
        const passwordInput = loginPage.find('input').at(1);

        usernameInput.get(0).value = 'Admin';
        passwordInput.get(0).value = '4dminPa55';

        loginPage.find('form').at(0).simulate('submit', { preventDefault() {} });
